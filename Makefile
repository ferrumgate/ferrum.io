...
LDFLAGS = -L$(shell pwd)/../external/libs/lib -luv -lssl -lcrypto -lnghttp2 -lhiredis
...
LDFLAGSTEST = -L$(shell pwd)/../external/libs/lib -lcmocka -luv -lpthread -lssl -lcrypto -lnghttp2 -lhiredis
...
ferrum : $(OBJS_REBRICK) $(OBJS_FERRUM)
...
test : $(OBJSTEST_REBRICK) $(OBJSTEST_FERRUM)
	$(CC) -o test  $(OBJSTEST_REBRICK) $(OBJSTEST_FERRUM) $(LDFLAGSTEST)
testrun: test
