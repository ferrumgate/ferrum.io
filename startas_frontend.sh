...
if [ $PROTOCOL_TYPE == "raw" ]; then
    sudo LOG_LEVEL=debug \
        REDIS_HOST=127.0.0.1:6379 \
        RAW_DESTINATION_HOST=192.168.88.51 \
        RAW_DESTINATION_TCP_PORT=5554 RAW_DESTINATION_UDP_PORT=5554 RAW_LISTEN_IP=127.0.0.1 \
        ...
        ROOT_FQDN=ferrumgate.zero PROTOCOL_TYPE=raw \
...
if [ $PROTOCOL_TYPE == "tproxy" ]; then
    ...
    yes | Yes | YES)
        ...
        # sudo iptables -t mangle -A PREROUTING -d 192.168.105.105/32 -p udp -j TPROXY --on-port 8081 --on-ip 127.0.0.1 --tproxy-mark 0x1/0x1
        ...
        ### testing
        # curl 192.168.105.105:9095
        # dig www.google.com @192.168.105.105 -p5554
        ### second option
        ...
        #iptables -t mangle -A OUTPUT -p tcp -m tcp --dport 9095 -j MARK --set-mark 1
    ...
    *) ;;
    ...
    sudo LOG_LEVEL=debug \
        ...
        RAW_DESTINATION_TCP_PORT=1000 RAW_DESTINATION_UDP_PORT=1000 RAW_LISTEN_IP=127.0.0.1 \
