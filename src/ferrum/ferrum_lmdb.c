...
int32_t ferrum_lmdb_new(ferrum_lmdb_t **lmdb, const char *path, const char *dbname, int32_t readonly, size_t maxdb, size_t maxsize) {
  ...
  if ((result = mdb_env_open(tmp->env, tmp->path, MDB_NOTLS | (readonly ? MDB_RDONLY : 0), 0664))) {
    ferrum_log_error("lmdb path %s could not open :%s", path, mdb_strerror(result));
    mdb_env_close(tmp->env);
    rebrick_free(tmp);
    return FERRUM_ERR_LMDB;
  }
  tmp->is_readonly = readonly;
  tmp->parent_trx = NULL;
  if ((result = mdb_txn_begin(tmp->env, tmp->parent_trx, (readonly ? MDB_RDONLY : 0), &tmp->trx))) {
    ferrum_log_error("lmdb path %s could not open :%s", path, mdb_strerror(result));
    mdb_env_close(tmp->env);
    rebrick_free(tmp);
    return FERRUM_ERR_LMDB;
  }
  if ((result = mdb_dbi_open(tmp->trx, dbname, MDB_DUPSORT | MDB_CREATE, &tmp->dbi))) {
    ferrum_log_error("lmdb path %s could not open :%s", path, mdb_strerror(result));
