...
/* static void replication_messages(redisAsyncContext *context, void *_reply, void *_privdata) {
  ...
  ferrum_policy_old_t *policy = cast(cmd->callback.arg1, ferrum_policy_old_t *);
...
static void redis_send_reset_replication_command(ferrum_policy_old_t *policy);

static void clear_policy_table(ferrum_policy_old_t *policy) {
  ferrum_policy_old_row_t *el, *tmp;
...
int32_t ferrum_policy_old_replication_message_execute(ferrum_policy_old_t *policy,
                                                      ferrum_policy_old_replication_message_t *msg) {
  ...
  if (!strcmp(msg->command, "update")) {
    ...
    ferrum_policy_old_row_t *el = NULL;
    HASH_FIND(hh, policy->table.rows, &client_id, sizeof(client_id), el);
    if (!el) {
      el = new1(ferrum_policy_old_row_t);
      constructor(el, ferrum_policy_old_row_t);
...
int32_t
ferrum_policy_old_replication_message_parse(char *str, ferrum_policy_old_replication_message_t *msg) {
  size_t count = 0;
  char *param;
...
static void execute_message(ferrum_policy_old_t *policy, int64_t time, int64_t id, char command[256]) {
  int32_t result;
  ...
  new2(ferrum_policy_old_replication_message_t, msg);
  result = ferrum_policy_old_replication_message_parse(command, &msg);
  ...
  result = ferrum_policy_old_replication_message_execute(policy, &msg);
  if (result) {
...
static void redis_send_alive_command(ferrum_policy_old_t *policy) {
...
static int32_t table_update_check(void *callback) {
  unused(callback);
  ferrum_policy_old_t *policy = cast(callback, ferrum_policy_old_t *);
...
int32_t ferrum_policy_old_new(ferrum_policy_old_t **policy, const ferrum_config_t *config) {
  ferrum_policy_old_t *tmp = new1(ferrum_policy_old_t);
  constructor(tmp, ferrum_policy_old_t);
  tmp->config = config;
...
int32_t ferrum_policy_old_destroy(ferrum_policy_old_t *policy) {
  if (policy) {
...
int32_t ferrum_policy_old_execute(const ferrum_policy_old_t *policy, uint32_t client_id, ferrum_policy_old_result_t *presult) {
  unused(policy);
  unused(client_id);
  unused(presult);
