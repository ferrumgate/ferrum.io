#include "./ferrum/ferrum_policy_old.h"
#include "cmocka.h"
#include <unistd.h>
...
static void test_ferrum_policy_old_replication_message_parse(void **start) {
  unused(start);

  int32_t result;
  new2(ferrum_policy_old_replication_message_t, msg);

  result = ferrum_policy_old_replication_message_parse(NULL, &msg);
  assert_int_not_equal(result, FERRUM_SUCCESS);

  result = ferrum_policy_old_replication_message_parse("", &msg);
  assert_int_not_equal(result, FERRUM_SUCCESS);

  result = ferrum_policy_old_replication_message_parse("test", &msg);
  assert_int_not_equal(result, FERRUM_SUCCESS);

  char *memory = malloc(250);

  strcpy(memory, "/");
  result = ferrum_policy_old_replication_message_parse(memory, &msg);
  assert_int_not_equal(result, FERRUM_SUCCESS);
...
static void test_ferrum_policy_old_replication_message_execute(void **start) {
  unused(start);
  ...
  ferrum_policy_old_t *policy;
  result = ferrum_policy_old_new(&policy, config);
  int32_t counter;
  ...
  ferrum_policy_old_destroy(policy);
  loop(counter, 100, TRUE);
  free(memory);
}
...
static void test_ferrum_policy_old_start(void **start) {
  unused(start);
...
static void test_ferrum_policy_old_execute(void **start) {
  unused(start);
  ...
  ferrum_policy_old_row_t *row1 = new1(ferrum_policy_old_row_t);
  constructor(row1, ferrum_policy_old_row_t);
  row1->client_id = 1;
  row1->is_drop = 1;
  ...
  new2(ferrum_policy_old_result_t, presult);
  result = ferrum_policy_old_execute(policy, 1, &presult);
  assert_int_equal(result, FERRUM_SUCCESS);
  ...
  result = ferrum_policy_old_execute(policy, 2, &presult);
  assert_int_equal(result, FERRUM_SUCCESS);
  assert_int_equal(presult.is_dropped, 1);
  assert_int_equal(presult.policy_number, 0);
...
int test_ferrum_policy_old(void) {
  const struct CMUnitTest tests[] = {
      cmocka_unit_test(test_ferrum_policy_old_replication_message_parse),
      cmocka_unit_test(test_ferrum_policy_old_replication_message_execute),
      cmocka_unit_test(test_ferrum_policy_old_start),
      cmocka_unit_test(test_ferrum_policy_old_execute),
