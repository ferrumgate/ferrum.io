#include "./rebrick/file/rebrick_filestream.h"
#include "cmocka.h"
#include <unistd.h>
...
static void filestream_sync_read(void **start) {
  ...
  result = rebrick_filestream_new(&file, "./test/rebrick/testdata/filestream_test.txt", O_RDWR, 0, NULL);
  if (result)
    result = rebrick_filestream_new(&file, "./rebrick/testdata/filestream_test.txt", O_RDWR, 0, NULL);
...
static void filestream_async_read(void **start) {
  ...
  file_closed = FALSE;
  // try to find file
  result = rebrick_filestream_new(&file, "./test/rebrick/testdata/filestream_test.txt", O_RDWR, 0, &callbacks);
  assert_true(result == 0);
  ...
  if (file_error) {
    ...
    file_written = FALSE;
    file_closed = FALSE;
    result = rebrick_filestream_new(&file, "./rebrick/testdata/filestream_test.txt", O_RDWR, 0, &callbacks);
    assert_true(result == 0);
    loop(counter, 100, TRUE);
  }
